...
const LeftSidebar = ({ isCollapsed, toggleSidebar }) => {
  ...
  const fetchRecent = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/left/latest-messages`, {
        headers: {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token")
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/left/search?query=${encodeURIComponent(search)}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
  ...
  return (
    <div className={sidebarClasses}>
      ...
      <div className="sidebar-content">
        ...
        {user && <div className="left-username">Hi, {user.username}</div>}
              ...
              {searching ? (
                ...
                <>
                  ...
                  <div className="search-timer-bar">
                    <div className="search-timer-bar-inner" style={{ width: `${timerPercent}%` }} />
                  </div>
                </>
              ) : searchResults.length > 0 ? (
                searchResults.map((user) => {
                  const recent = recentChats.find((rc) => rc.user._id === user._id && rc.latestMessage)
                  return (
                    <div key={user._id} className="search-dropdown-item" onClick={() => handleSelectUser(user)}>
                      <div className="chat-avatar">{user.username.slice(0, 2).toUpperCase()}</div>
          ...
          {Array.isArray(recentChats) && recentChats.filter((rc) => rc.latestMessage).length > 0 ? (
            recentChats
              ...
              .map(({ user: chatUser, latestMessage }) => (
                ...
                >
                  <div className="chat-avatar">{chatUser.username.slice(0, 2).toUpperCase()}</div>
                  <div className="chat-info">
            ...
            <div className="search-status">
              Start chatting to see recent chats here.
              <br />
