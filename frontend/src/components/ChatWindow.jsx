...
const ChatWindow = ({ leftSidebarCollapsed, rightSidebarCollapsed }) => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/left/messages/${selectedChat._id}`, {
        headers: {
  ...
  useEffect(() => {
    ...
    const handleSeen = ({ by }) => {
      setMessages((prev) =>
        prev.map((m) => (m.sender === user._id && m.receiver === selectedChat._id ? { ...m, seen: true } : m)),
      )
    }
  ...
  }, [socket, selectedChat, user, setMessages])
  ...
  const contactOnline = selectedChat && onlineUsers.includes(selectedChat._id)
  ...
  useEffect(() => {
    ...
    const handleMessagesDeleted = ({ messageIds }) => {
      setMessages((prev) =>
        prev.map((msg) =>
          messageIds.includes(msg._id) ? { ...msg, content: "[ deleted message ]", deleted: true, edited: false } : msg,
        ),
      )
    }
  ...
  useEffect(() => {
    ...
    const handleEditMessage = (updatedMsg) => {
      setMessages((prev) =>
        prev.map((m) => (m._id === updatedMsg._id ? { ...m, content: updatedMsg.content, edited: true } : m)),
      )
    }
    ...
    >
      <div className="chat-header">
        <div className="chat-contact-avatar">
          {selectedChat.username ? selectedChat.username.slice(0, 2).toUpperCase() : ""}
        </div>
        <div>
          <div className="chat-contact-name">{selectedChat.username || selectedChat.name}</div>
          <div className={`chat-contact-status ${contactOnline ? "online" : "offline"}`}>
            {contactOnline ? "Online" : "Offline"}
          </div>
        </div>
        <div className="chat-header-actions">
          <button className="menu-btn" onClick={() => setMenuOpen((prev) => !prev)} tabIndex={0} aria-label="Open menu">
          ...
          {menuOpen && (
            <div className="chat-header-menu">
              ...
              >
                <svg width="18" height="18" fill="#888" viewBox="0 0 20 20" style={{ marginRight: 6 }}>
                  <rect x="3" y="3" width="14" height="14" rx="3" fill="#e0e0e0" />
                  <path d="M7 10.5l2 2 4-4" stroke="#888" strokeWidth="2" fill="none" strokeLinecap="round" />
                </svg>
                Select Messages
              </button>
            </div>
      ...
      <div className="chat-messages">
        {Object.keys(groupedMessages).map((date) => (
          <div key={date}>
            ...
            {groupedMessages[date].map((msg) => {
              ...
              const showCheckbox = selectingForSummary || (selectionMode && isSent)
              return (
                <div
                  key={msg._id}
                ...
                >
                  ...
                  <div className={`message-col ${isSent ? "align-end" : "align-start"}`}>
                    <div className="message-row">
                      ...
                      >
                        {msg.deleted || msg.content === "[ deleted message ]" ? "[ deleted message ]" : msg.content}
                      </div>
                    </div>
                    <div className={`message-meta ${isSent ? "align-end" : "align-start"}`}>
                      {isSent && msg.edited && (
                        <>
                          ...
                          <span className="dot-separator" style={{ margin: "0 4px", color: "#888" }}>
                            &#8226;
                          </span>
                        </>
                      )}
                        ...
                        <span className="read-receipt">
                          {msg.seen ? (
                            <svg width="16" height="16" fill="#4caf50" viewBox="0 0 16 16">
                              <path d="M1.5 8.5l4 4 9-9" stroke="#4caf50" strokeWidth="2" fill="none" />
                              <path d="M5.5 8.5l2 2 5-5" stroke="#4caf50" strokeWidth="2" fill="none" />
                            </svg>
                          ) : (
                            <svg width="16" height="16" fill="#888" viewBox="0 0 16 16">
                              <path d="M1.5 8.5l4 4 9-9" stroke="#888" strokeWidth="2" fill="none" />
                            </svg>
                          )}
                        </span>
                      )}
        ...
        {typingUsers.includes(selectedChat._id) && <div className="typing-indicator">Typing...</div>}
      ...
      {selectionMode && (
        <div className="delete-actions-bar">
          <button onClick={handleDeleteMessages} className="delete-btn" disabled={selectedMessages.length === 0}>
            Delete Selected
          </button>
            ...
            <div className="edit-modal-actions">
              <button
                ...
                onClick={async () => {
                  ...
                  const token = localStorage.getItem("token")
                  const res = await fetch(`${process.env.REACT_APP_API_URL}/api/left/edit-message/${msgId}`, {
                    method: "PUT",
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: `Bearer ${token}`,
                  ...
                  if (res.ok) {
                    ...
                    setMessages((prev) =>
                      prev.map((m) => (m._id === updated._id ? { ...m, content: updated.content, edited: true } : m)),
                    )
                    setEditMode(false)
                    setSelectionMode(false)
                    setSelectedMessages([])
              ...
              <button className="edit-modal-cancel" onClick={() => setEditMode(false)}>
                Cancel
              </button>
            </div>
          </div>
